...
// Barcodes are pre-printed externally, not generated by this app. Each
// location gets its real-world barcode value assigned once in Admin Panel,
// stored at sites/{site}/locationBarcodes/{groupKey}. The scan gate looks
// that value up and checks the scan against it.
function renderLocationScanGate(container, site, groupKey, groupName, onVerified) {
  container.innerHTML = `<div class="card"><p style="color:var(--muted);">Loading...</p></div>`;

  db.ref(`sites/${site}/locationBarcodes/${groupKey}`).once("value").then((snap) => {
    const expectedCode = snap.exists() ? String(snap.val()).trim() : null;

    if (!expectedCode) {
      container.innerHTML = `<div class="panel-placeholder">No barcode has been assigned to ${groupName} yet. Ask your Super User to assign one in Admin Panel.</div>`;
      return;
    }

    container.innerHTML = `
      <div class="card">
        <h4 style="color:var(--navy); margin-bottom:8px;">Scan the barcode posted at ${groupName}</h4>
    ...
    function handleResult(decoded) {
      if (decoded.trim() === expectedCode) {
        ...
        statusEl.style.color = "var(--danger)";
        statusEl.textContent = "That barcode doesn't match this location. Make sure you're scanning the code posted here.";
      }
    }
